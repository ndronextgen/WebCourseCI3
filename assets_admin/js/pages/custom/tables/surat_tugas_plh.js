jQuery(document).ready(function () {
	...
	var tblTugasPlh = $("#tblTugasPlh").KTDatatable({
		// datasource definition
		data: {
			type: "remote",
			source: {
				read: {
					url: url + "/api/surat_tugas_plh/listing",
					method: "POST",
				},
			},
			pageSize: 20,
...
function buatSuratTugasPlh(id) {
	...
	frm.attr("action", url + "admin/surat_tugas_plh/add");
...
function lihatSuratTugasPlh(id) {
	let frm = $("#frmSuratTugasPlh");
	frm.attr("action", url + "admin/surat_tugas_plh/detail");
	frm.attr("method", "post");
	$("#id_pegawai").val(id);
