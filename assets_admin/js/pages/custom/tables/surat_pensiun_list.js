...
jQuery(document).ready(function () {
  ...
  var tblPensiunDetailList = $('#tblPensiunDetailList').KTDatatable({
    // datasource definition
    data: {
      type: 'remote',
      source: {
        read: {
          url: url+'/api/surat_pensiun/datatable_list',
          method: 'POST',
          params: {
    ...
    columns: [
      {
        ...
        template: function(row, index) {
          var currPage = tblPensiunDetailList.getCurrentPage();
      ...
      }, 
      {
        ...
        template: function(row) {
          ...
          if (row.id_status_srt == 3) {
            edit += `<button type="button" class="btn btn-outline-hover-primary" onclick="download_surat_finished(${row.id_surat_pensiun});"><i class="flaticon-download"></i> Download</button>`;
          }
