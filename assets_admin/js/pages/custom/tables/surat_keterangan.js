		...
		columns: [
			...
			},
			{
				...
				template: function (row) {
					...
					if (row.jml <= 0) {
						data =
							'<span style="font-weight:bold;">' + row.jenis_surat + "</span>";
					} else {
						data =
			...
			},
			{
				...
				template: function (row) {
					let data;
					if (row.jml <= 0) {
						data = '<span style="font-weight:bold;">' + row.nama + "</span>";
					} else {
						data = '<span style="font-weight:italic;">' + row.nama + "</span>";
					}
			...
			},
			{
				...
				template: function (row) {
					...
					if (row.jml <= 0) {
						data =
							'<span style="font-weight:bold;">' + row.tgl_surat + "</span>";
					} else {
						data =
							'<span style="font-weight:italic;">' + row.tgl_surat + "</span>";
					}
			...
			},
			{
				...
				template: function (row) {
					...
					idStatus = parseInt(row.id_status);
					switch (idStatus) {
						case 0:
							data =
								'<span class="badge badge-status" onclick="showTimeline(\'' +
								row.id_srt +
						...
						case 21:
							if (row.is_dinas == 1) {
								data =
									...
									row.fontcolor +
									';">' +
									"Menunggu Verifikasi<br>Kepala Subkoordinator<br>Kepegawaian" +
									"</span>";
							} else {
								data =
									'<span class="badge badge-status" onclick="showTimeline(\'' +
									...
									row.fontcolor +
									';">' +
									"Menunggu Verifikasi<br>Kepala Subbagian" +
									"</span>";
							}
							break;
						case 22:
						...
						default:
							data =
								'<span class="badge badge-dark badge-status" onclick="showTimeline(\'' +
								row.id_srt +
								"')\">" +
								row.nama_status_next +
								"</span>";
