			...
			},
			{
				...
				template: function (row) {
					...
					if (row.id_status_srt == 3 && row.group_dinas == 2) {
						edit += `<button type="button" class="btn btn-outline-hover-primary" onclick="download_surat(${row.id_surat_pensiun});"><i class="flaticon-download"></i> UPT to Dinas</button><button type="button" class="btn btn-outline-hover-primary" onclick="download_surat2(${row.id_surat_pensiun});"><i class="flaticon-download"></i> Dinas to BKD</button>`;
...
function download_surat2(id) {
	let url = getCookie("url");
	url += "admin/surat_pensiun/download_surat2/" + id;
	window.open(url, "_blank");
	window.focus();
}
