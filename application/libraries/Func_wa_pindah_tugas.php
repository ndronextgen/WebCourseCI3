...
function Notifikasi_data($Pindah_tugas_id){
    ...
    $Query_data_surat = $CI->db->query("SELECT
                                        ...
                                        LEFT JOIN (
                                            ...
                                            WHERE a.Id != '' AND Pindah_tugas_id = '$Pindah_tugas_id'")->row();
    return $Query_data_surat;
}

function Notifikasi_admin_penerima($Pindah_tugas_id){
    $CI = &get_instance();
    # Query_notif_beta
...
function notif_pt_admin_tambah($Pindah_tugas_id){
    $CI = &get_instance();
    ...
    $data_penerima  = $CI->func_wa_pindah_tugas->Notifikasi_data($Pindah_tugas_id);
    $Hari 			= $CI->func_wa_pindah_tugas->gethari(date('l', strtotime($data_penerima->Updated_at)));
    $Tanggal_indo 	= $Hari . ' ' . $CI->func_wa_pindah_tugas->tgl_indonesia($data_penerima->Updated_at);
    $Jam			= date("H:i:s", strtotime($data_penerima->Updated_at));

    $Sapaan 		= $CI->func_wa_pindah_tugas->get_sapaan(strtotime(date("H:i:s", strtotime($data_penerima->Updated_at))));
    $data_admin     = $CI->func_wa_pindah_tugas->Notifikasi_admin_penerima($data_penerima->Pindah_tugas_id);
    $data_kasubag   = $CI->func_wa_pindah_tugas->Notifikasi_kasubag();
    $data_sekdis    = $CI->func_wa_pindah_tugas->Notifikasi_sekdis();


// ---- to pembuat surat
    $message_pembuat = 'Hai <b>' . $data_penerima->nama_lengkap . '</b>, <br/>
    
Selamat Anda telah berhasil Tambah <b>Surat Kebutuhan Pindah Tugas</b> dalam aplikasi Si-ADiK. <br/><br/>

<b>Surat	 	 : Kebutuhan Pindah Tugas</b> <br/>
<b>Tanggal 		 : ' . $Tanggal_indo . ' ' . $Jam . ' WIB</b> <br/>
<b>Url 		 	 : https://dcktrp.jakarta.go.id/si-adik</b> <br/>
<b>Status	 	 : ' . $data_penerima->status . '</b> <br/>
'.$CI->func_wa_pindah_tugas->Get_footer().'
';
...
' . $Sapaan . ' Pemberitahuan Aplikasi Si-ADiK Terdapat <b>Surat Kebutuhan Pindah Tugas</b> baru, Mohon agar dilakukan verifikasi ketahap selanjutnya.<br/><br/>
...
function notif_pt_update($id_surat){
    $CI = &get_instance();
    $CI->load->helper('send_email');
    $CI->load->helper('wa');
    // ---------------------------
    $tgl_now = date('Y-m-d H:i:s');
    $data_penerima  = $CI->func_wa_pindah_tugas->Notifikasi_data($id_surat);
    $Hari 			= $CI->func_wa_pindah_tugas->gethari(date('l', strtotime($data_penerima->Updated_at)));
    $Tanggal_indo 	= $Hari . ' ' . $CI->func_wa_pindah_tugas->tgl_indonesia($data_penerima->Updated_at);
    $Jam			= date("H:i:s", strtotime($data_penerima->Updated_at));
    $Sapaan 		= $CI->func_wa_pindah_tugas->get_sapaan(strtotime(date("H:i:s", strtotime($data_penerima->Updated_at))));
    ...
    $data_kasubag   = $CI->func_wa_pindah_tugas->Notifikasi_kasubag();
    $data_sekdis    = $CI->func_wa_pindah_tugas->Notifikasi_sekdis();
    $data_sudinupt  = $CI->func_wa_pindah_tugas->Notifikasi_kasubag_sudinupt($data_penerima->lokasi_kerja_pegawai);
    // --------------------------
...
' . $Sapaan . ' Pemberitahuan Aplikasi Si-ADiK terhadap usulan <b>Surat Kebutuhan Pindah Tugas</b> anda.<br/><br/>
...
Selamat Anda telah berhasil Verifikasi <b>Surat Kebutuhan Pindah Tugas</b> dalam aplikasi Si-ADiK. <br/><br/>
...
' . $Sapaan . ' Pemberitahuan Aplikasi Si-ADiK terhadap usulan <b>Surat Kebutuhan Pindah Tugas</b>.<br/><br/>
...
' . $Sapaan . ' Pemberitahuan Aplikasi Si-ADiK terhadap usulan <b>Surat Kebutuhan Pindah Tugas</b> anda, Surat keterangan sudah dapat diunduh<br/><br/>
