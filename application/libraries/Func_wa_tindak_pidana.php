...
function Notifikasi_data($Tindak_pidana_id){
    ...
    $Query_data_surat = $CI->db->query("SELECT
                                        ...
                                        LEFT JOIN (
                                            ...
                                            WHERE a.Id != '' AND Tindak_pidana_id = '$Tindak_pidana_id'")->row();
    return $Query_data_surat;
}

function Notifikasi_admin_penerima($Tindak_pidana_id){
    $CI = &get_instance();
    # Query_notif_beta
...
function notif_tp_admin_tambah($Tindak_pidana_id){
    $CI = &get_instance();
    ...
    $data_penerima  = $CI->func_wa_tindak_pidana->Notifikasi_data($Tindak_pidana_id);
    $Hari 			= $CI->func_wa_tindak_pidana->gethari(date('l', strtotime($data_penerima->Updated_at)));
    $Tanggal_indo 	= $Hari . ' ' . $CI->func_wa_tindak_pidana->tgl_indonesia($data_penerima->Updated_at);
    $Jam			= date("H:i:s", strtotime($data_penerima->Updated_at));

    $Sapaan 		= $CI->func_wa_tindak_pidana->get_sapaan(strtotime(date("H:i:s", strtotime($data_penerima->Updated_at))));
    $data_admin     = $CI->func_wa_tindak_pidana->Notifikasi_admin_penerima($data_penerima->Tindak_pidana_id);
    $data_kasubag   = $CI->func_wa_tindak_pidana->Notifikasi_kasubag();
    $data_sekdis    = $CI->func_wa_tindak_pidana->Notifikasi_sekdis();


// ---- to pembuat surat
    $message_pembuat = 'Hai <b>' . $data_penerima->nama_lengkap . '</b>, <br/>
    
Selamat Anda telah berhasil Tambah <b>Surat Keterangan Bebas Tindak Pidana</b> dalam aplikasi Si-ADiK. <br/><br/>

<b>Prihal Surat 	 : Surat Keterangan Bebas Tindak Pidana</b> <br/>
<b>Tanggal 		 : ' . $Tanggal_indo . ' ' . $Jam . ' WIB</b> <br/>
<b>Url 		 	 : https://dcktrp.jakarta.go.id/si-adik</b> <br/>
<b>Status	 	 : ' . $data_penerima->status . '</b> <br/>
'.$CI->func_wa_tindak_pidana->Get_footer().'
';
    ...
    $objSendMail = sendMail([
        'subject' => 'Si-ADiK (Surat Tindak Pidana)',
...
' . $Sapaan . ' Pemberitahuan Aplikasi Si-ADiK Terdapat <b>Surat Keterangan Bebas Tindak Pidana</b> baru, Mohon agar dilakukan verifikasi ketahap selanjutnya.<br/><br/>
...
function notif_tp_update($id_surat){
    $CI = &get_instance();
    $CI->load->helper('send_email');
    $CI->load->helper('wa');
    // ---------------------------
    $tgl_now = date('Y-m-d H:i:s');
    $data_penerima  = $CI->func_wa_tindak_pidana->Notifikasi_data($id_surat);
    $Hari 			= $CI->func_wa_tindak_pidana->gethari(date('l', strtotime($data_penerima->Updated_at)));
    $Tanggal_indo 	= $Hari . ' ' . $CI->func_wa_tindak_pidana->tgl_indonesia($data_penerima->Updated_at);
    $Jam			= date("H:i:s", strtotime($data_penerima->Updated_at));
    $Sapaan 		= $CI->func_wa_tindak_pidana->get_sapaan(strtotime(date("H:i:s", strtotime($data_penerima->Updated_at))));
    ...
    $data_kasubag   = $CI->func_wa_tindak_pidana->Notifikasi_kasubag();
    $data_sekdis    = $CI->func_wa_tindak_pidana->Notifikasi_sekdis();
    $data_sudinupt  = $CI->func_wa_tindak_pidana->Notifikasi_kasubag_sudinupt($data_penerima->lokasi_kerja_pegawai);
    // --------------------------
...
' . $Sapaan . ' Pemberitahuan Aplikasi Si-ADiK terhadap usulan <b>Surat Keterangan Bebas Tindak Pidana</b> anda.<br/><br/>
...
Selamat Anda telah berhasil Verifikasi <b>Surat Keterangan Bebas Tindak Pidana</b> dalam aplikasi Si-ADiK. <br/><br/>
...
' . $Sapaan . ' Pemberitahuan Aplikasi Si-ADiK terhadap usulan <b>Surat Keterangan Bebas Tindak Pidana</b>.<br/><br/>
...
' . $Sapaan . ' Pemberitahuan Aplikasi Si-ADiK terhadap usulan <b>Surat Keterangan Bebas Tindak Pidana</b> anda, Surat keterangan sudah dapat diunduh<br/><br/>
