...
class Master_pelatihan extends CI_Controller {
	...
	public function index()
	{
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator")
		{
			...
			$config['base_url'] = base_url() . 'admin/master_pelatihan/index/';
			...
			$this->load->view('dashboard_admin/master/header_master');
		}
		else
		{
	...
	public function edit()
	{
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator")
		{
			$id['id_master_pelatihan'] = $this->uri->segment(4);
			$q = $this->db->get_where("tbl_master_pelatihan",$id);
			$d = array();
			foreach($q->result() as $dt)
			{
				$d['id_param'] = $dt->id_master_pelatihan;
	...
	public function cari()
	{
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator")
		{
			...
			$config['base_url'] = base_url() . 'admin/master_pelatihan/cari/';
			$config['total_rows'] = $tot_hal->num_rows();
			$config['per_page'] = $limit;
			$config['uri_segment'] = 4;
			$config['first_link'] = 'Awal';
	...
	public function hapus()
	{
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator")
		{
			...
			header('location:'.base_url().'admin/master_pelatihan');
		}
		else
		{
			header('location:'.base_url().'');
	...
	public function simpan()
	{
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator")
		{
			...
			$id['id_master_pelatihan'] = $this->input->post("id_param");
			...
			else
			{
				...
				if($st=="edit")
				{
					...
					redirect('admin/master_pelatihan');
				}
				else if($st=="tambah")
				{
