...
class Master_eselon extends CI_Controller {
	...
	public function edit()
	{
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator")
		{
			...
			$d['credit'] = $this->config->item('credit_aplikasi');
			$d['alamat'] = $this->config->item('alamat_instansi');
			$d['page_name'] = 'Ubah Master Eselon';
			$d['menu_open'] = 'master';
			$id['id_eselon'] = $this->uri->segment(4);
	...
	public function detail()
	{
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator")
		{
			...
			$d['credit'] = $this->config->item('credit_aplikasi');
			$d['alamat'] = $this->config->item('alamat_instansi');
			$d['page_name'] = 'Detail Master Eselon';
			$d['menu_open'] = 'master';
	...
	public function tambah()
	{
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator")
		{
			...
			$d['credit'] = $this->config->item('credit_aplikasi');
			$d['alamat'] = $this->config->item('alamat_instansi');
			$d['page_name'] = 'Tambah Master Eselon';
			$d['menu_open'] = 'master';
			$d['id_param'] = "";
			$d['nama_eselon'] = ""; 
			$d['level'] = ""; 
	...
	public function hapus()
	{
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator")
		{
			...
			header('location:'.base_url().'admin/master_eselon');
		}
		else
		{
			header('location:'.base_url().'');
