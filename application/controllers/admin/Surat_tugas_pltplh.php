...
class Surat_tugas_pltplh extends CI_Controller {
	...
	public function index()
	{
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator")
		{
			if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator") {
				...
				$d['page_name'] = 'Surat Keterangan Tugas PLT/PLH';
				$d['deskripsi'] = 'Pilih Pegawai Yang Akan ditugaskan Menjadi PLT/PLH';
				$d['menu_open'] = 'kk';
	
				$this->load->view('dashboard_admin/kertas_kerja/surat_tugas_pltplh/home',$d);
			}
			else {
	...
	public function add() {
		...
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator" && $id_pegawai!=0)
		{
			...
			$d['credit'] = $this->config->item('credit_aplikasi');
			$d['alamat'] = $this->config->item('alamat_instansi');
			$d['page_name'] = 'Tambah Surat Keterangan Tugas PLT/PLH';
			$d['menu_open'] = 'kk';
			$d['act'] = 'add';
			...
			$this->load->view('dashboard_admin/kertas_kerja/surat_tugas_pltplh/form',$d);
		}
		else
		{
			header('location:'.base_url());
	...
	public function simpan() {
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator") {
			if ($this->input->post('id_pegawai') != 0) {
				...
				$d['credit'] = $this->config->item('credit_aplikasi');
				$d['alamat'] = $this->config->item('alamat_instansi');
				$d['page_name'] = 'Ubah Surat Keterangan Tugas PLT/PLH';
				$d['menu_open'] = 'kk';
				...
				if ($this->input->post('id_surat_tugas_pltplh') != 0) {
					//edit mode
					//get data surat Tugas PLH old
					$q = $this->db->get_where("tbl_data_surat_tugas_pltplh", ['id_surat_tugas_pltplh' => $this->input->post('id_surat_tugas_pltplh')]);
					foreach ($q->result() as $p) {
						$d['surat'] = $p;
					...
					if ($d['surat'] != null) {
						...
						$d['alasan_pltplh'] = $this->input->post('alasan_pltplh');
						$d['type_surat'] = $this->input->post('type_surat');
						$d['tembusan'] = $tembusan;
					}
					else {
				...
				if ($this->input->post('alasan_pltplh') == '') {
					$d['page_name'] = 'Ubah Surat Keterangan Tugas PLT/PLH';
				...
				elseif ($d['surat'] != null) {
					...
					if ($this->db->where('id_surat_tugas_pltplh', $this->input->post('id_surat_tugas_pltplh'))->update('tbl_data_surat_tugas_pltplh', $in)) {
						header('location:'.base_url().'admin/surat_tugas_pltplh');
					}
					else {
					...
					if ($this->db->insert('tbl_data_surat_tugas_pltplh', $in)) {
						header('location:'.base_url().'admin/surat_tugas_pltplh');
					}
					else {
	...
	public function detail() {
		...
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator") {
			...
			$d['credit'] = $this->config->item('credit_aplikasi');
			$d['alamat'] = $this->config->item('alamat_instansi');
			$d['page_name'] = 'Detail Surat Keterangan Tugas PLT/PLH';
			$d['menu_open'] = 'kk';
			$d['act'] = 'detail';
			...
			$this->load->view('dashboard_admin/kertas_kerja/surat_tugas_pltplh/detail',$d);
		}
		else {
			header('location:'.base_url().'');
		}
	...
	public function cetak($id_surat=0,$id_pegawai=0) {
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator" && $id_surat!=0) {
			...
			foreach ($q->result() as $p) {
				...
				if($p->type_surat=='PLT') {
					$this->load->view('dashboard_admin/kertas_kerja/surat_tugas_pltplh/export_plt',$d);
				} else if($p->type_surat=='PLH') {
					$this->load->view('dashboard_admin/kertas_kerja/surat_tugas_pltplh/export',$d);
				} else {
					$this->load->view('dashboard_admin/kertas_kerja/surat_tugas_pltplh/export',$d);
				}
	...
	// 	if ($surat != null) {
	// 		$this->db->where('id_surat_tugas_pltplh', $id_surat)->delete('tbl_data_surat_tugas_pltplh');
	// 		header('location:'.base_url().'admin/surat_tugas_pltplh/detail/'.$p->id_pegawai);
	// 	}
	// 	else {
	// 		header('location:'.base_url().'admin/surat_tugas_pltplh');
	// 	}
...
/* End of file surat_tugas_pltplh.php */
/* Location: ./application/controllers/surat_tugas_pltplh.php */