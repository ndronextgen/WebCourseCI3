...
class Arsip_pribadilainnya extends CI_Controller
{
	public function __construct()
	{
		parent::__construct();
		$this->load->model('arsip_pribadilainnya_model', 'arsip_pribadi_model');
	...
	public function pribadi_datatables()
	{
		...
		foreach ($list as $r) {
			...
			$path_folder = 'asset/upload/pribadi/pribadi_' . $r->id_data_keluarga . '_' . $r->id_arsip_pribadi;
			$path_file    = $path_folder . '/' . $r->file_name;
			$is_file_exist = file_exists($path_folder);

			if ($is_file_exist) {
				$file = $this->func_table->get_file($path_file, $r->file_name_ori);
			} else {
				$file = '-';
				goto skip_row;
			}
			...
			//add html for action
			//$row[] = '<a class="btn btn-sm btn-default" href="javascript:void(0)" title="Lihat" onclick="lihat_file_pribadi('."'".$r->id_arsip_pribadi."'".')"><i class="glyphicon glyphicon-pencil"></i> Lihat File</a> <button class="btn btn-sm btn-default" data-toggle="modal" data-target="#modal-download-pribadi" data-id="'.utf8_encode($r->id_arsip_pribadi).'" data-title="Download" title="Download Data"><i class="fa fa-download"></i> Download</button>';
			...
			if ($is_file_exist) {
				$button .= '&nbsp;<button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#modal-download-pribadi" data-id="' . utf8_encode($r->id_arsip_pribadi) . '" data-title="Download" title="Download Data"><i class="fa fa-download"></i></button>';
			}
			$row[] = $button;
