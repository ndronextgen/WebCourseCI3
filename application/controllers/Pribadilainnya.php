...
class Pribadilainnya extends CI_Controller {
	public function __construct() {
		parent::__construct();
		$this->load->helper('file');
		$this->load->model('pribadilainnya_model', 'tbl_data_pribadilainnya');
		$this->load->model('arsip_pribadi_model');
	}
	
	public function pribadilainnya_datatables() {
        // Datatables Variables
		$id = $this->session->userdata('id_pegawai');
		$list = $this->tbl_data_pribadilainnya->get_datatables($id);
		$data = array();
		$no = $_POST['start'] + 1;
		...
		foreach ($list as $r) {
			...
			//add html for action
			$button = '	<a class="btn btn-sm btn-primary" href="javascript:void(0);" title="Edit" onclick="edit_keluarga('."'".$r->id_arsip_pribadi."'".')"><i class="glyphicon glyphicon-pencil"></i> Edit</a>
			...
			var_dump($arsip);
			if ($arsip) {
				$button .= '&nbsp;<button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#modal-download-arsip" data-key="keluarga" data-id="' . utf8_encode($arsip->id_arsip_pribadi) . '" data-title="Download" title="Download Data"><i class="fa fa-download"></i> Download</button>';
			} else {
				goto skip_row;
			}
			$row [] = $button;
	...
	public function pribadilainnya_list($id) {
		...
		$list = $this->db->query("SELECT * FROM tbl_arsip_pribadi WHERE id_data_keluarga NOT IN (
							SELECT id_data_keluarga FROM tbl_data_keluarga
						) AND tbl_arsip_pribadi.created_id  = '$id' AND id_data_keluarga='0'
